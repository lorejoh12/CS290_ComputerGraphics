...
# Convert x,y,z rectangular coordinates to a polar array
...
# Convert polar to a rectangular array
...
def getSamplePointsFrom2DHistogram(numSamples, histogram):
    ...
    for j in range(numSections):
        ...
        for i in range(0, len(randomSectorIndexes)):
            ...
            sampleNum+=1
    return Ps
# Takes in a number of spherical harmonics and a resolution on the sphere and returns a F matrix
...
# NHarmonics: the number of spherical harmonics, NSpheres: the number of spherical shells to use,
# SPointRange: The array of spherical samplings to use on the distributed point cloud.
def getSphericalHarmonicMagnitudes(Ps, Ns, NHarmonics, NSpheres, SPointRange):
    ...
    # Return the approximated mesh as well as the original sampled matrix
